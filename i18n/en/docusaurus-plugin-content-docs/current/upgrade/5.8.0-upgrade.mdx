...
```bash
nerdctl exec -it rainbond-allinone bash
curl https://rainbond-script.oss-cn-hangzhou.aliyuncs. com/upgrade-5.8.0.sh | bash
```
...
```bash
nerdctl stop rainbond-allinone && nerdctl rm rainbond-allinone
#This command parameter needs to be the same as the rainbond-allinone container started before
nerdctl run -d -p 7070:7070 -v ~/.ssh:/root/.ssh -v ~/ rainbonddata:/app/data \
...
```bash
nerdctl run -it --rm -v /:/rootfs registry.cn-hangzhou.aliyuncs.com/goodrain/rbd-grctl:v5.8.0-release copy
