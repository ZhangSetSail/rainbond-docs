...
1. 机器上需要安装 containerd 服务以及 nerdctl 工具
...
```shell
nerdctl pull rainbond/rainbond:v5.8.1-dind-allinone
nerdctl save -o rainbond-v5.8.1-dind-allinone.tar rainbond/rainbond:v5.8.1-dind-allinone
```

导入 tar 包到目标服务器中。

```shell
nerdctl load -i rainbond-v5.8.1-dind-allinone.tar
```
...
```bash
mkdir -p ~/containerddata 
mkdir -p /opt/rainbond
mkdir -p ~/rainbonddata
nerdctl run --privileged -d -p 7070:7070 -p 80:80 -p 443:443 -p 6060:6060 -p 8443:8443 \
--name=rainbond-allinone --restart=on-failure \
-v ~/.ssh:/root/.ssh \
-v ~/rainbonddata:/app/data \
-v /opt/rainbond:/opt/rainbond \
-v ~/containerddata:/var/lib/con \
-e EIP=$EIP \
rainbond/rainbond:v5.8.1-dind-allinone \
&& nerdctl logs -f rainbond-allinone
```
...
```shell
nerdctl pull rainbond/rainbond:v5.8.0-arm64-dind-allinone
nerdctl save -o rainbond-v5.8.0-arm64-dind-allinone.tar rainbond/rainbond:v5.8.0-arm64-dind-allinone
```

导入 tar 包到目标服务器中。

```shell
nerdctl load -i rainbond-v5.8.0-arm64-dind-allinone.tar
```
...
-v ~/rainbonddata:/app/data \
-v /opt/rainbond:/opt/rainbond \
-v ~/containerddata:/var/lib/containerddata \
-e EIP=$EIP \
